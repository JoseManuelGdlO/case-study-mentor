...
import type { DashboardExamDate } from '@/types';
...
type Props = {
  activeExamDate: DashboardExamDate | null;
};

const CountdownTimer = ({ activeExamDate }: Props) => {
  const [timeLeft, setTimeLeft] = useState<TimeLeft | null>(null);

  useEffect(() => {
    if (!activeExamDate) return;
    const calculate = () => {
      const diff = new Date(activeExamDate.date).getTime() - Date.now();
  ...
  if (!activeExamDate || !timeLeft) return null;
        ...
        <div className="flex flex-col sm:flex-row items-center">
          <div className="gradient-primary p-4 sm:p-5 flex items-center gap-3 w-full sm:w-auto">
            ...
            <div className="text-primary-foreground">
              <p className="font-bold text-sm">{activeExamDate.name}</p>
