...
casesRouter.get('/', authenticate, requireCaseEditor(), validateQuery(listCasesQuerySchema), async (req, res, next) => {
    try {
...
casesRouter.post('/bulk-upload', authenticate, requireAdmin(), upload.single('file'), async (req, res, next) => {
    try {
        const file = req.file;
        if (!file?.buffer) {
