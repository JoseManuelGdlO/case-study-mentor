...
import { completeStudyPlanTaskSchema, createWellbeingInterventionSchema, upsertWellbeingTodaySchema, } from '../schemas/study-plan.schema.js';
...
studyPlanRouter.post('/wellbeing/today', authenticate, validateBody(upsertWellbeingTodaySchema), async (req, res, next) => {
    try {
        if (!req.user)
            throw new Error('No user');
...
studyPlanRouter.post('/wellbeing/interventions', authenticate, validateBody(createWellbeingInterventionSchema), async (req, res, next) => {
    try {
        if (!req.user)
            throw new Error('No user');
