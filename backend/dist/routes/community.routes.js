...
communityRouter.post('/threads/:threadId/posts', authenticate, validateBody(createCommunityPostSchema), async (req, res, next) => {
    try {
        if (!req.user)
            throw new Error('No user');
...
communityRouter.patch('/posts/:postId/moderation', authenticate, requireCaseEditor(), validateBody(moderateCommunityPostSchema), async (req, res, next) => {
    try {
