...
import { createCollaboratorCode, deleteCollaboratorCode, listCollaboratorCodes, setCollaboratorCodeActive, updateCollaboratorCodeDisplayName, } from '../services/collaborator-code.service.js';
...
backofficeRouter.post('/phrases', requireCaseEditor(), validateBody(phraseCreateSchema), async (req, res, next) => {
    try {
...
backofficeRouter.put('/phrases/:id', requireCaseEditor(), validateBody(phraseUpdateSchema), async (req, res, next) => {
    try {
...
backofficeRouter.post('/flashcards', requireCaseEditor(), validateBody(flashcardCreateSchema), async (req, res, next) => {
    try {
        const b = req.body;
...
backofficeRouter.put('/flashcards/:id', requireCaseEditor(), validateBody(flashcardUpdateSchema), async (req, res, next) => {
    try {
...
backofficeRouter.post('/exam-dates', requireCaseEditor(), validateBody(examDateCreateSchema), async (req, res, next) => {
    try {
...
backofficeRouter.put('/exam-dates/:id', requireCaseEditor(), validateBody(examDateUpdateSchema), async (req, res, next) => {
    try {
        const b = req.body;
...
backofficeRouter.post('/collaborator-codes', requireAdmin(), validateBody(collaboratorCodeCreateSchema), async (req, res, next) => {
    try {
        const body = req.body;
...
backofficeRouter.patch('/collaborator-codes/:id', requireAdmin(), validateBody(collaboratorCodePatchSchema), async (req, res, next) => {
    try {
        const { isActive } = req.body;
...
backofficeRouter.put('/collaborator-codes/:id', requireAdmin(), validateBody(collaboratorCodeDisplayNameSchema), async (req, res, next) => {
    try {
...
backofficeRouter.get('/users', requireAdmin(), validateQuery(backofficeUsersQuerySchema), async (req, res, next) => {
    try {
        ...
        const completedByUser = profileIds.length > 0
            ...
            : [];
        const examsCompletedMap = new Map(completedByUser.map((r) => [r.userId, r._count._all]));
...
function adminPlanChangePasswordMatches(input, expected) {
    const a = input ?? '';
    if (a.length !== expected.length)
        return false;
    return timingSafeEqual(Buffer.from(a, 'utf8'), Buffer.from(expected, 'utf8'));
}
backofficeRouter.patch('/users/:id', requireAdmin(), validateBody(backofficeUserUpdateSchema), async (req, res, next) => {
    try {
        ...
        const planChangeRequested = requestedTier !== undefined && requestedTier !== effective.plan;
        if (planChangeRequested) {
            const pwd = body.confirmationPassword;
            if (pwd === undefined || pwd.length === 0) {
                const err = new Error('Se requiere la contraseña de confirmación para cambiar el plan del usuario');
                err.status = 400;
                throw err;
            }
            if (!adminPlanChangePasswordMatches(pwd, env.ADMIN_PLAN_CHANGE_PASSWORD)) {
                const err = new Error('Contraseña de confirmación incorrecta');
                err.status = 403;
                throw err;
            }
        }
        ...
        if (newEmail !== undefined && newEmail !== profile.email) {
            if (profile.authProvider !== 'email') {
                const err = new Error('Solo se puede cambiar el correo de cuentas registradas con email y contraseña');
                err.status = 400;
                throw err;
            }
        ...
        const nextRolesSorted = roleList !== undefined ? [...new Set(roleList)].sort().join(',') : prevRolesSorted;
        ...
        if (planChangeRequested && requestedTier !== undefined) {
            if (requestedTier === 'free') {
                ...
                const pricePlan = await getActiveSubscriptionPlanForTier(requestedTier);
                const now = new Date();
...
backofficeRouter.get('/stats', requireAdmin(), async (_req, res, next) => {
    try {
        const pairKey = (specialtyId, areaId) => `${specialtyId}\0${areaId}`;
        const [totalUsers, totalExams, totalCases, totalQuestions, totalPublishedCases, specialties, allByPair, publishedByPair,] = await Promise.all([
        ...
        const allMap = new Map(allByPair.map((r) => [pairKey(r.specialtyId, r.areaId), r._count._all]));
        const pubMap = new Map(publishedByPair.map((r) => [pairKey(r.specialtyId, r.areaId), r._count._all]));
        const catalogAreaKeys = new Set();
