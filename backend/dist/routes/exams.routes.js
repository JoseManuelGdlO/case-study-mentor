...
examsRouter.post('/generate', authenticate, validateBody(generateExamSchema), async (req, res, next) => {
    try {
        if (!req.user)
...
examsRouter.put('/:id/answer', authenticate, validateBody(examAnswerSchema), async (req, res, next) => {
    try {
        if (!req.user)
            throw new Error('No user');
...
examsRouter.post('/:id/feedback', authenticate, validateBody(submitExamFeedbackSchema), async (req, res, next) => {
    try {
        if (!req.user)
            throw new Error('No user');
...
examsRouter.put('/:id/complete', authenticate, validateBody(examTimeSchema), async (req, res, next) => {
    try {
        if (!req.user)
            throw new Error('No user');
        const result = await examService.completeExam(req.user.id, paramString(req.params.id), req.body.timeSpentSeconds);
        await invalidateUserStats(req.user.id);
        res.json(result);
    }
    catch (e) {
