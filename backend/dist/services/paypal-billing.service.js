...
export async function ensurePayPalBillingPlan(planRow) {
    ...
    const billingPlan = (await createPlanRes.json());
...
export async function syncProfileFromPayPalSubscriptionResource(sub) {
    ...
    let expiresAt = next ? new Date(next) : null;
    if (!expiresAt) {
...
export async function createPayPalSubscriptionCheckout(userId, tier, collaboratorCodeId) {
    ...
    if (!approve)
        throw serviceError('PayPal no devolvió enlace de aprobación de suscripción', 502);
    return { approvalUrl: approve };
}
export async function confirmPayPalSubscriptionAfterApproval(userId, subscriptionId) {
...
export async function cancelPayPalSubscriptionForUser(userId) {
    const profile = await prisma.profile.findUnique({
        where: { id: userId },
        select: { paypalSubscriptionId: true },
    });
    ...
    const res = await paypalFetch(`/v1/billing/subscriptions/${encodeURIComponent(profile.paypalSubscriptionId)}/cancel`, {
        method: 'POST',
...
//# sourceMappingURL=paypal-billing.service.js.map