...
function getStripe() {
    if (!env.STRIPE_SECRET_KEY)
        throw serviceError('Stripe no está configurado', 503);
    if (!stripeSdk) {
        stripeSdk = new Stripe(env.STRIPE_SECRET_KEY);
    }
    return stripeSdk;
}
function isStripeDuplicatePromotionCodeError(err) {
    const e = err;
    if (e?.code === 'resource_already_exists')
        return true;
    const m = (e?.message ?? '').toLowerCase();
    return (m.includes('already been taken') ||
...
async function deleteStripePromotionCodeRemote(stripePromotionCodeId) {
    const key = env.STRIPE_SECRET_KEY;
    if (!key)
        return;
    const res = await fetch(`https://api.stripe.com/v1/promotion_codes/${encodeURIComponent(stripePromotionCodeId)}`, {
        method: 'DELETE',
...
async function createStripeCouponAndPromotionCode(normalized, percentOff, maxRedemptions, validUntil, promoActive) {
...
async function deactivateAndDropStripePromotion(stripePromotionCodeId, stripeCouponId) {
    const stripe = getStripe();
    try {
...
async function createStripeCouponAndPromotionCodeForReplace(existing, normalized, percentOff, maxRedemptions, validUntil, nextIsActive) {
    const stripe = getStripe();
    const coupon = await stripe.coupons.create({
        percent_off: percentOff,
        duration: 'once',
    ...
    try {
        ...
        if (sameText && isStripeDuplicatePromotionCodeError(err)) {
            ...
            try {
                ...
                throw serviceError(`${msg} Revisa Stripe o crea un código nuevo en el backoffice.`, 502);
            }
        }
        else {
            await stripe.coupons.del(coupon.id).catch(() => undefined);
...
export async function updatePromotionCode(id, input) {
    const row = await prisma.promotionCode.findUnique({ where: { id } });
    if (!row)
        throw serviceError('Código promocional no encontrado', 404);
    const { normalized, maxRedemptions, validFrom, validUntil } = validatePromotionInput(input);
    ...
    const { coupon, promotion } = await createStripeCouponAndPromotionCodeForReplace({
        ...
        code: row.code,
    }, normalized, input.percentOff, maxRedemptions, validUntil, nextIsActive);
    try {
        const updated = await prisma.promotionCode.update({
            where: { id },
            data: {
...
export async function deletePromotionCode(id) {
    const row = await prisma.promotionCode.findUnique({ where: { id } });
    if (!row)
        throw serviceError('Código promocional no encontrado', 404);
...
export async function setPromotionCodeActive(id, isActive) {
    const row = await prisma.promotionCode.findUnique({ where: { id } });
    if (!row)
        throw serviceError('Código promocional no encontrado', 404);
    const stripe = getStripe();
...
async function assertCheckoutEligible(row, userId) {
    ...
    if (profile.stripeSubscriptionId) {
        throw serviceError('Los códigos de promoción solo aplican a nuevas suscripciones con Stripe. Ya tienes una suscripción activa.', 409);
    }
    if (!row.isActive) {
...
export async function resolvePromotionCodeForCheckout(userId, rawCode) {
...
export async function validatePromotionCodeForUser(userId, rawCode) {
...
//# sourceMappingURL=promotion-code.service.js.map