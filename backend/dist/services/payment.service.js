...
export async function processStripeWebhook(rawBody, signature) {
    if (!env.STRIPE_WEBHOOK_SECRET)
        throw serviceError('Webhook Stripe no configurado', 503);
    if (!signature)
        throw serviceError('Firma Stripe ausente', 400);
    const stripe = getStripe();
    let event;
    try {
...
export async function createPayPalOrder(userId, tier) {
    ...
    if (!approve)
        throw serviceError('PayPal no devolvió enlace de aprobación', 502);
