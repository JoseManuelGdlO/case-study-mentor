...
export async function syncProfileFromPayPalSubscriptionResource(sub: PayPalSubscriptionApi): Promise<void> {
  ...
  if (status === 'EXPIRED') {
    ...
    if (!row) return;
    if (row.paypalSubscriptionId && row.paypalSubscriptionId !== sub.id) return;
  ...
  if (status === 'CANCELLED') {
    ...
    if (!profile) return;
    if (profile.paypalSubscriptionId && profile.paypalSubscriptionId !== sub.id) return;

    let accessUntil: Date | null = null;
    if (next) {
      const nd = new Date(next);
      if (nd > now) accessUntil = nd;
    }
