...
export async function listPendingMentorReviews(pageInput?: number, limitInput?: number) {
...
export async function getMentorReviewExamDetail(examId: string) {
  const exam = await prisma.exam.findFirst({
    where: { id: examId, mentorReviewEligible: true, status: 'completed' },
    include: { user: { select: { email: true, firstName: true, lastName: true } } },
  });
  if (!exam) {
    const err = new Error('Examen no encontrado o no elegible para revisión') as Error & { status: number };
    err.status = 404;
    throw err;
  }
...
export async function submitMentorReview(
  ...
  if (!exam) {
    const err = new Error('Examen no pendiente de revisión') as Error & { status: number };
    err.status = 404;
    throw err;
  }
