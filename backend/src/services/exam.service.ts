...
export type GenerateExamInternalOptions = {
  onboardingWelcomeSnapshots?: { firstName: string; university: string };
};

function buildWelcomeQuestionLeadIn(firstName: string, university: string): string {
...
export async function generateExam(
  userId: string,
  input: GenerateInput,
  internal?: GenerateExamInternalOptions
) {
  const {
    language,
    mode,
  ...
  if (!hasPaidPlan && questionCount > FREE_TRIAL_MAX_QUESTIONS) {
    const err = new Error(
      `En el plan gratuito cada examen tiene como máximo ${FREE_TRIAL_MAX_QUESTIONS} preguntas. Suscríbete para crear exámenes más largos.`,
  ...
  if (!hasPaidPlan && profile.freeTrialExamsUsed >= FREE_TRIAL_MAX_EXAMS) {
    const err = new Error(
      'Ya usaste los 2 exámenes de prueba gratuitos (10 preguntas cada uno). Suscríbete para seguir creando exámenes.',
    ) as Error & { status: number };
    err.status = 403;
    throw err;
  }
 ...
 */
export async function createOnboardingWelcomeExam(
  userId: string,
...
export async function getExamById(userId: string, examId: string) {
  ...
  return {
    data: {
      ...
      flatQuestions,
      mentorReview:
        exam.mentorReviewedAt != null && exam.mentorReviewRating != null
          ? {
