...
export async function getThreadById(id: string, roles: string[]) {
...
export async function createPost(
  userId: string,
  threadId: string,
  body: { body: string; parentPostId?: string | null }
) {
  const thread = await prisma.communityThread.findUnique({ where: { id: threadId } });
  if (!thread || thread.isHidden) throw notFound('Hilo no encontrado');
  if (thread.isLocked) {
    const err = new Error('Este hilo está bloqueado') as Error & { status: number };
    err.status = 400;
    throw err;
  }
...
export async function moderatePost(postId: string, isHidden: boolean) {
  const existing = await prisma.communityPost.findUnique({ where: { id: postId }, select: { id: true } });
  if (!existing) throw notFound('Respuesta no encontrada');
