...
import {
  ...
  wellbeingPushUnsubscribeSchema,
} from '../schemas/profile.schema.js';
import * as profileService from '../services/profile.service.js';
import {
  getUserVapidPublicKey,
  isUserPushConfigured,
  getUserWellbeingNotificationPreferences,
  subscribeUserPush,
  unsubscribeUserPush,
  updateUserWellbeingNotificationPreferences,
} from '../services/user-push.service.js';
...
profileRouter.post(
  '/wellbeing-notifications/subscription',
  authenticate,
  validateBody(wellbeingPushSubscribeSchema),
  async (req, res, next) => {
    try {
      if (!req.user || !req.actor) throw new Error('No user');
      if (req.actor.id !== req.user.id) {
...
profileRouter.delete(
  '/wellbeing-notifications/subscription',
  authenticate,
  validateBody(wellbeingPushUnsubscribeSchema),
  async (req, res, next) => {
    try {
      if (!req.user || !req.actor) throw new Error('No user');
      if (req.actor.id !== req.user.id) {
...
profileRouter.patch(
  '/wellbeing-notifications/preferences',
  authenticate,
  validateBody(wellbeingNotificationPreferencesSchema),
  async (req, res, next) => {
    try {
      if (!req.user || !req.actor) throw new Error('No user');
      if (req.actor.id !== req.user.id) {
